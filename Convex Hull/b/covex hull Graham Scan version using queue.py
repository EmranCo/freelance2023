...
def convex_hull(points: List[Tuple[float, float]]) -> List[Tuple[float, float]]:
    ...
    hull = queue.Queue()
    hull.put(sorted_points[0])
    hull.put(sorted_points[1])
    hull.put(sorted_points[2])
    for i in range(3, len(sorted_points)):
        while hull.qsize() > 1 and cross(hull.queue[-2], hull.queue[-1], sorted_points[i]) <= 0:
            hull.get()
        hull.put(sorted_points[i])
    return list(hull.queue)
